...
'''
    item_based_predict = pd.read_csv('item_based_with_sim_adcos/item_based_u%dresult.csv' % i,
                                     names=['userid', 'itemid', 'predict_rating'])
    item_based_merge = pd.merge(item_based_predict, df_real, on=['userid', 'itemid'])
    item_based_merge = item_based_merge.drop('timestamp', axis=1)
    item_based_merge['mae'] = abs(item_based_merge['predict_rating'] - item_based_merge['real_rating'])
    ...
    item_based_predict = pd.read_csv('item_based_with_sim_pearson/item_based_u%dresult.csv' % i,
                                     names=['userid', 'itemid', 'predict_rating'])
    item_based_merge = pd.merge(item_based_predict, df_real, on=['userid', 'itemid'])
    item_based_merge = item_based_merge.drop('timestamp', axis=1)
    item_based_merge['mae'] = abs(item_based_merge['predict_rating'] - item_based_merge['real_rating'])
...
'''plt.plot(range(1,6),lst_item,label='item_based_with_adcos')
plt.plot(range(1,6),lst_item,label='item_based_with_pearson')'''
